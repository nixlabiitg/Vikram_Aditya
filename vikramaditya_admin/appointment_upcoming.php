...
<div class="main-content p-4 sm:p-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <h1 class="text-2xl font-bold text-[#7A3E00]">Upcoming Appointment</h1>
    ...
    <div class="bg-[#FFF9F0] shadow rounded-lg p-4 mb-6 border border-[#FFD699] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <input type="text" placeholder="Search Appointment." class="border border-gray-400 rounded px-4 py-2 w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-400" />
        <select class="border border-gray-400 rounded px-4 py-2 w-full sm:w-auto focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-orange-400">
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p><strong>Family:</strong> <div x-data="{ open: false }">
                                  ...
                                  <div 
                                    x-show="open" 
                                    class="fixed inset-0 item-center justify-center pointer-events-none bg-black/30 bg-opacity-40 z-40"
                                    @click="open = false"
                                    x-transition
                                  ></div>

                                  <div x-show="open" x-transition 
                                    class="fixed top-20 bg-white rounded-lg shadow-lg z-50 w-full max-w-2xl">
...
<script>
    ...
    paymentBtn.addEventListener("click", () => {
        if (!paymentBtn.dataset.status) { // only open if not already paid/exempted
...
function filterAppointments() {
    ...
    const statusFilter = document.querySelectorAll('select')[0].value.toLowerCase();
